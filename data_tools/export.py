...
class DataExportTool(BaseDataTool):
    ...
    async def execute(self, client: ZenMoneyClient, args: Dict[str, Any]) -> CallToolResult:
        ...
        if transaction_type == 'income':
            ...
            filtered = [t for t in filtered if hasattr(t, 'is_expense') and t.is_expense]
        elif transaction_type == 'transfer':
            filtered = [t for t in filtered if hasattr(t, 'is_transfer') and t.is_transfer]
        ...
        if not filtered:
            return CallToolResult(
                content=[TextContent(type="text", text="❌ Нет транзакций для экспорта")]
            )
